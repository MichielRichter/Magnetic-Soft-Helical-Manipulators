classdef CosseratHelix
    ...
    methods
        ...
        function obj = CosseratHelix(length, radius_outer, radius_inner, ...
            ...
            % Derived
            obj.height = radius_outer - radius_inner;
            obj.pitch = radius_outer * (angle_bending_design / number_of_windings);
            obj.width = (length - number_of_windings * obj.pitch) / (2 + number_of_windings);
            obj.phi = (obj.width / (obj.width+obj.pitch)) * 2*pi;
            
            % Elastic modulus for composite
            volume_core = pi * (radius_core_outer^2 - radius_core_inner^2) * length;
